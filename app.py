...
DATABASE_URL = f"postgresql:+asyncpg://course_python_db_b2sb_user:pRhvIBmJ8HjRofYJjgMufrTJ9ezTWZ42@dpg-csn17vo8fa8c73adprkg-a.oregon-postgres.render.com/course_python_db_b2sb"
...
#Controller for HTTP Headers / HTTP Body(nur Ausgabe)
# Methode 1: GET: Retrieve all items
@app.get('/items', response_model=List[Item]) #geben liste mit items zurück 
async def get_items(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ItemDB))
    items = result.scalars().all()
    return items

# Methode 2: GET: Retrieve item by ID --> wird auf dem PC ausgeführt, code liegt zb. auf server
